import React__default, { createContext, useState, useEffect, useRef, useContext, createElement } from 'react';
...
const GlobalProvider = ({
  ...
  const handleAction = (id, edit) => {
    if (edit) {
      ...
      let indexOfId = lodash.indexOf(editArrCopy, id);
      ...
      let indexOfId = lodash.indexOf(replyArrCopy, id);
  ...
  const onEdit = (text, comId, parentId) => {
    ...
    if (parentId) {
      const indexOfParent = lodash.findIndex(copyData, {
        comId: parentId
      });

      const indexOfId = lodash.findIndex(copyData[indexOfParent].replies, {
        comId: comId
      });
      ...
      const indexOfId = lodash.findIndex(copyData, {
        comId: comId
      });
  ...
  const onReply = (text, comId, parentId, uuid) => {
    let copyData = [...data];
...
const RegularInput = ({
  ...
  return React__default.createElement("form", {
    ...
    type: 'button',
    onClick: () => mode === 'editMode' ? globalStore.handleAction(comId, true) : globalStore.handleAction(comId, false)
...
const AdvancedInput = ({
  ...
  return React__default.createElement("div", {
    ...
    style: globalStore.formStyle || formStyle,
    onSubmit: async e => editText != '<p></p>' ? (await handleSubmit(e, editText), setEditor(EditorState.createEmpty())) : null
  }, React__default.createElement("div", {
    ...
    style: globalStore.submitBtnStyle || submitBtnStyle,
    onClick: async e => editText != '<p></p>' ? (await handleSubmit(e, editText), setEditor(EditorState.createEmpty())) : null
  }, "Post")))));
};
...
const InputField = ({
  ...
  const editMode = async advText => {
    const textToSend = advText ? advText : text;
    return await globalStore.onEdit(textToSend, comId, parentId), globalStore.onEditAction && (await globalStore.onEditAction({
  ...
  const replyMode = async (replyUuid, advText) => {
    const textToSend = advText ? advText : text;
    return await globalStore.onReply(textToSend, comId, parentId, replyUuid), globalStore.onReplyAction && (await globalStore.onReplyAction({
      userId: globalStore.currentUserData.currentUserId,
  ...
  const submitMode = async (createUuid, advText) => {
    const textToSend = advText ? advText : text;
    return await globalStore.onSubmit(textToSend, createUuid), globalStore.onSubmitAction && (await globalStore.onSubmitAction({
      userId: globalStore.currentUserData.currentUserId,
      comId: createUuid,
      avatarUrl: globalStore.currentUserData.currentUserImg,
      userProfile: globalStore.currentUserData.currentUserProfile ? globalStore.currentUserData.currentUserProfile : null,
  ...
  const handleSubmit = async (event, advText) => {
...
const DeleteModal = ({
  ...
  return React__default.createElement("div", null, React__default.createElement("div", {
    ...
    className: 'delete',
    onClick: async () => (await globalStore.onDelete(comId, parentId), globalStore.onDeleteAction && (await globalStore.onDeleteAction({
      comIdToDelete: comId,
...
const CommentSection = ({
  ...
  }), globalStore.data.length > 0 ? globalStore.data.map(i => {
    return React__default.createElement("div", {
      key: i.comId
      ...
      logIn: logIn
    }), i.replies && i.replies.length > 0 && i.replies.map(j => {
      return React__default.createElement("div", {
        className: 'replySection',
        key: j.comId
      }, React__default.createElement(CommentStructure, {
...
export { CommentSection$1 as CommentSection };
